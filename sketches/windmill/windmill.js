...
class Mill {
    constructor(_radius, _speed, _midpoint, _angle) {
        // Constructor for the line.
        this.angle = _angle;
        this.radius = _radius; // Radius of the "unit circle" which correlates to the length of the line.
        this.vectorA = createVector(
            round(this.radius * cos(this.angle)),
            -round(this.radius * sin(this.angle))
        );
        this.vectorB = createVector(
            -round(this.radius * cos(this.angle)),
            round(this.radius * sin(this.angle))
        );
        ...
        if (this.vectorA.x != this.vectorB.x) {
    ...
    spin() {
        if (this.angle <= 359) {
            ...
            this.vectorA = createVector(
                round(this.radius * cos(this.newAngle)),
                -round(this.radius * sin(this.newAngle))
            );
            this.vectorB = createVector(
                -round(this.radius * cos(this.newAngle)),
                round(this.radius * sin(this.newAngle))
